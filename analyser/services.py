...
class AnalyserService:
    ...
    def iniciar_analise(self):
        ...
        try:
            ...
            self._log('SUCCESS', f'✅ Análise concluída! {len(dados_extraidos)} campos extraídos em {self.resultado.tempo_processamento}')
    ...
    def _baixar_do_sharepoint(self, arquivo_info):
        ...
        """
        try:
            # TODO: Implementar integração real com SharePoint
            from sharepoint.services import SharePointService
            
            sp_service = SharePointService()
            
            conteudo = sp_service.download_file(
                folder_id=self.caso.sharepoint_folder_id,
                item_id=arquivo_info['id']
            )
            
            return conteudo
            
        except Exception as e:
            logger.error(f"Erro ao baixar arquivo do SharePoint: {str(e)}")
            
            # ⚠️ MOCK PARA TESTES - REMOVER EM PRODUÇÃO
            self._log('WARNING', f'⚠️ Usando arquivo mock para teste (SharePoint não configurado)')
            
            # Retorna PDF mock para testes
...
"""
        ...
        for i, campo in enumerate(campos, 1):
            ...
            # ✅ Dicas específicas por tipo (adaptado aos novos tipos)
            if campo['tipo'] == 'DATA':
...
⚠️ IMPORTANTE: Você DEVE responder APENAS com um JSON válido, sem nenhum texto adicional antes ou depois.
...
9. ✅ Não inclua markdown (```json) na resposta, apenas o JSON puro
...
"""
        ...
        prompt += "\n\n**Agora analise os documentos e retorne o JSON com os dados extraídos.**"
    ...
    def _analisar_com_gemini(self, prompt, arquivos):
        ...
        try:
            ...
            generation_config = genai.GenerationConfig(
                ...
                top_p=0.8,
                top_k=40,
                max_output_tokens=getattr(settings, 'GEMINI_MAX_TOKENS', 4096),
            )
...
"""
    ...
    def aplicar_ao_caso(self):
        ...
        if self.resultado.resumo_caso:
            try:
                if hasattr(self.caso, 'resumo_caso'):
                    self.caso.resumo_caso = self.resultado.resumo_caso
                    self.caso.save()
                    self._log('SUCCESS', '✅ Resumo do caso atualizado')
                else:
                    self._log('INFO', 'ℹ️ Campo resumo_caso não existe no modelo Caso')
            except Exception as e:
        ...
        # Resumo final
        total = len(campos)
        self._log('SUCCESS', f'✅ Aplicação concluída! {campos_aplicados}/{total} campos atualizados, {campos_ignorados} não encontrados, {campos_com_erro} erros')
    ...
    def _atualizar_campo_personalizado(self, campo_id, valor):
        ...
        # ✅ Conversão conforme o tipo (adaptado aos novos tipos)
        ...
        # ✅ Atualiza ou cria (apenas para campos NÃO repetíveis ligados ao caso)
        ValorCampoPersonalizado.objects.update_or_create(
            caso=self.caso,
            campo=campo,
            instancia_grupo=None,  # ✅ IMPORTANTE: campos diretos não têm grupo
    ...
    def _criar_evento_fluxo_interno(self, campos_aplicados, total_campos):
        ...
        try:
            ...
            # Modelo EventoFluxoInterno não existe
            pass
        except Exception as e:
